...
commander.version(version, '-v, --version')
  ...
  .action(async (projectName, cmd) => {
    ...
    inquirer.prompt(promptTypeList).then(result => {
      ...
      console.log("您选择的模版类型信息如下，看仔细了啊：" + val);
      console.log('项目初始化拷贝获取中，别催了，在加速了...');
      if (!url) {
        console.log(chalk.red(`${val} 该类型暂不支持，哦豁...`));
        process.exit(1);
      }
      exec('git clone ' + url, function (error, stdout, stderr) {
        ...
        fs.rename(gitName, projectName, (err) => {
          if (err) {
            ...
            console.log(chalk.green(`The ${projectName} project template successfully create(恭喜这位倒霉蛋，项目模版创建成功！)`));
          }
        });
      });
    })
